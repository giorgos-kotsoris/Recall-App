...
class AuthService:
    ...
    def create_user(self, username, password, role):
        ...
        self._validate_password(password, required=True); self._validate_role(role)
        if self.users.by_username(username):
            raise ValueError("Υπάρχει ήδη χρήστης με αυτό το username.")
    ...
    def update_own_credentials(self, user_id, username, password=None):
        ...
        username = (username or "").strip()
        self._validate_username(username);
        self._validate_password(password, required=False)
    ...
    def update_user_role(self, user_id, role):
        ...
        user.role = role; db.session.commit()
        return user

    def delete_user(self, user_id, current_user_id):
        if user_id == current_user_id:
            raise ValueError("Δεν μπορείτε να διαγράψετε τον λογαριασμό με τον οποίο είστε συνδεδεμένοι.")
        user = self.users.get(user_id)
        if not user: raise LookupError("Ο χρήστης δεν βρέθηκε.")
    ...
    def _validate_username(username):
        if len(username) < 3:
            raise ValueError("Το username πρέπει να έχει τουλάχιστον 3 χαρακτήρες.")

    @staticmethod
    def _validate_password(password, required):
        if required and not password:
            raise ValueError("Ο κωδικός είναι υποχρεωτικός.")
    ...
    def _validate_role(role):
        if role not in {"admin", "viewer"}: 
            raise ValueError("Μη έγκυρος ρόλος χρήστη.")

    def ensure_default_admin(self):
        default_username = "Ύπαρχος"
        if self.users.by_username(default_username):
            return
        legacy = self.users.by_username("admin")
        if legacy and legacy.role == "admin":
            legacy.username = default_username; db.session.commit();
            return
        if not legacy:
            user = UserModel(username=default_username, role="admin");
