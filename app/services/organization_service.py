...
class OrganizationService:
    ...
    def list_members(self, group_id=None):
        return self.members.all(group_id)
    ...
    def delete_group(self, group_id):
        ...
        if not group:
            raise LookupError("Η ομάδα δεν βρέθηκε.")
        if group.members:
            raise ValueError("Δεν μπορείτε να διαγράψετε ομάδα που έχει μέλη.")
        self.groups.delete(group)

    def create_member(self, data):
        entity = Member(**data)
        entity.validate()
        if not self.groups.get(entity.group_id):
            raise ValueError("Η επιλεγμένη ομάδα δεν υπάρχει.")
    ...
    def update_member(self, member_id, data):
        ...
        if not member:
            raise LookupError("Το μέλος δεν βρέθηκε.")
        if not self.groups.get(entity.group_id):
            raise ValueError("Η επιλεγμένη ομάδα δεν υπάρχει.")
